...
template <typename TIn, typename TOut>
Status reduce_mean_matrix_columns(cudaStream_t stream, const TIn* input, TOut* output, int m, int n, void* buffer, size_t buffer_size) {
  return detail::call_reduce_matrix_columns<TIn, TOut, Identity, Identity, true>(
      stream, input, output, m, n, buffer, buffer_size);
}

#define INSTANTIATE_REDUCE_MEAN_MATRIX_COLUMNS(T) \
  template Status reduce_mean_matrix_columns<T, T>(cudaStream_t stream, const T* input, T* output, int m, int n, void* buffer, size_t buffer_size)
