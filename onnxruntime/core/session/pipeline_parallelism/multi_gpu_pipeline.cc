  ...
  while (req_processed < num_reqs) {
    ...
    if (exec_frame.stage_id == 0) {  // this means we've reached step > 0
      ...
      if (step_id == num_steps) {  // we're done with all steps of this request, move the output
        ...
        auto& posn_ids = exec_frame.next_step_input_buffer_map[model_ensemble_cfg.logits_name].data;
        ...
        token_ptr->ort_value_names = {model_ensemble_cfg.input_ids_name, model_ensemble_cfg.position_ids_name};
...
void PipelineSession::Validate(const ModelEnsembleConfig& model_ensemble_cfg) {
  ...
  for (size_t i = 0, end = model_ensemble_cfg.model_config_vec.size(); i < end; ++i) {
    ...
    if (i == end - 1) {
      ORT_ENFORCE(output_names_set.count(model_ensemble_cfg.logits_name) == 1,
                  "Name: ", model_ensemble_cfg.logits_name, " is not a valid output name.");
    }
  }
}
